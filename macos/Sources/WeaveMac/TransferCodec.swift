...
enum TransferLimits {
    static let maxSubscriptions = 64
    static let maxNameBytes = 320
    static let maxSourceBytes = 8 * 1024
    static let maxPayloadBytes = 5 * 1024 * 1024
    static let maxPlaintextBytes = 20 * 1024 * 1024
    static let maxCiphertextBytes = maxPlaintextBytes + 64
}

struct TransferLink: Equatable {
    let host: String
    let port: UInt16
    let token: String
    let key: Data
    ...
    static func parse(_ raw: String) throws -> TransferLink {
        guard
        ...
        guard
            ...
            throw WeaveMacError.message("局域网地址、端口或密钥无效")
        }
...
enum TransferCodec {
    ...
    static func encode(_ subscriptions: [TransferSubscription]) throws -> Data {
        guard !subscriptions.isEmpty, subscriptions.count <= TransferLimits.maxSubscriptions else {
            throw WeaveMacError.message("请选择 1–\(TransferLimits.maxSubscriptions) 个订阅")
        }
        ...
        for subscription in subscriptions {
            ...
            guard output.count <= TransferLimits.maxPlaintextBytes else {
                throw WeaveMacError.message("传输内容超过 20 MiB 限制")
            }
        }
        return output
    }

    static func decode(_ data: Data) throws -> [TransferSubscription] {
        guard data.count <= TransferLimits.maxPlaintextBytes else {
            throw WeaveMacError.message("传输内容过大")
        }
        var reader = BinaryReader(data: data)
        guard try reader.read(count: 8) == plaintextMagic else {
            throw WeaveMacError.message("传输内容标识无效")
        }
        let count = Int(try reader.readUInt32())
        guard count > 0, count <= TransferLimits.maxSubscriptions else {
            throw WeaveMacError.message("订阅数量无效")
        }
        ...
        guard reader.isAtEnd else {
            throw WeaveMacError.message("传输内容包含多余数据")
        }
        return result
    }

    static func seal(_ plaintext: Data, key: Data) throws -> Data {
        guard key.count == 32 else { throw WeaveMacError.message("传输密钥长度无效") }
        ...
        guard let combined = box.combined else {
            throw WeaveMacError.message("无法生成加密传输包")
        }
        return encryptedMagic + combined
    }

    static func open(_ packet: Data, key: Data) throws -> Data {
        guard
            ...
            throw WeaveMacError.message("加密传输包无效")
        }
...
private struct BinaryReader {
    ...
    mutating func read(count: Int) throws -> Data {
        guard count >= 0, offset <= data.count - count else {
            throw WeaveMacError.message("传输内容被截断")
        }
        defer { offset += count }
    ...
    mutating func readString(maxBytes: Int) throws -> String {
        ...
        guard count <= maxBytes else { throw WeaveMacError.message("传输字段过大") }
        let bytes = try read(count: count)
        guard let string = String(data: bytes, encoding: .utf8) else {
            throw WeaveMacError.message("传输文本不是有效 UTF-8")
        }
        return string
    }
}
...
private extension Data {
    ...
    mutating func appendString(_ value: String, maxBytes: Int) throws {
        ...
        guard bytes.count <= maxBytes else { throw WeaveMacError.message("传输字段过大") }

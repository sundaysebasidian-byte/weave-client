...
> 当前里程碑是 **Android 0.3.0-alpha27 / macOS 0.1.0-alpha05**。Android 四个 ABI 的
> CMFA/Mihomo
...
- DNS 可选择 DoH 或 DoT，并提供普通隐私、广告过滤、家庭过滤和自定义加密端点；过滤模式会阻断常见浏览器 Secure DNS 绕过，并保留 fake-IP 全隧道接管；IPv4-only 模式会停用 IPv6 DNS，

...
- Manifest 禁止明文流量：`usesCleartextTraffic=false`。
- 不允许任意 CA 绕过、跳过证书校验或“兼容模式”静默降级。
- 订阅默认只接受 HTTPS 和用户主动选择的本地文件。
...
- 局域网互传默认关闭，只在用户点击生成后监听，成功读取一次或 5 分钟即失效；不开放远程
  控制或内核调试 API。
...
- 订阅 URL、认证信息和备份密钥单独加密；节点展示模型不含凭据。
...
- macOS 订阅库使用 Keychain 中的 256-bit 主密钥和 AES-GCM 加密文件；局域网互传的随机
  密钥不写入订阅库。
...
- 禁止订阅配置声明任意本地文件、任意脚本、外部 UI 或管理端口。
...
日志事件使用结构化 allowlist，不在写入后再靠正则“尽量脱敏”。

...
Android 第二阶段在不改变 WVLAN001 字节格式的前提下增加了三项控制面能力：发送端可按订阅勾选，
二维码扫描后接收端显示链接，导入时必须输入发送端单独显示的 6 位带外确认短码；Android 接收
legacy 包中同名且同源的订阅，会先运行 Subscription Guard，再原位更新并保留固定节点 ID。
因此 macOS/iOS 仍可使用 v1 编码互传；它们不需要理解 Android 的本地同步 ID。

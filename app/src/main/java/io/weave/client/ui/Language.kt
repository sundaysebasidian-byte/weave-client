...
private val SUPPLEMENTAL_TRANSLATIONS = mapOf(
    ...
    "复制" to SupplementalTranslation("Copy", "複製", "コピー", "Copier", "Kopieren"),
    "重定向不代表最终站点可达" to SupplementalTranslation("A redirect does not verify the final destination", "重新導向不代表最終站點可達", "リダイレクトは最終宛先への到達を証明しません", "Une redirection ne confirme pas l’accès à la destination finale", "Eine Weiterleitung bestätigt nicht die Erreichbarkeit des Ziels"),
    "脱敏检测摘要" to SupplementalTranslation("Redacted diagnostic summary", "去識別化檢測摘要", "匿名化した診断概要", "Résumé de diagnostic expurgé", "Bereinigte Diagnoseübersicht"),
    "预览脱敏摘要" to SupplementalTranslation("Preview redacted summary", "預覽去識別化摘要", "匿名化した概要を確認", "Aperçu du résumé expurgé", "Bereinigte Übersicht ansehen"),
    "已复制脱敏摘要" to SupplementalTranslation("Redacted summary copied", "已複製去識別化摘要", "匿名化した概要をコピーしました", "Résumé expurgé copié", "Bereinigte Übersicht kopiert"),
    "无法写入剪贴板，请重试" to SupplementalTranslation("Could not copy. Please try again.", "無法寫入剪貼簿，請重試", "コピーできませんでした。再試行してください", "Copie impossible. Réessayez.", "Kopieren fehlgeschlagen. Bitte erneut versuchen."),
    "网络已变化，请重新测速" to SupplementalTranslation("Network changed. Run a new latency test.", "網路已變更，請重新測速", "ネットワークが変わりました。再測定してください", "Le réseau a changé. Relancez le test.", "Netzwerk geändert. Bitte erneut messen."),
    "仅包含检测状态与耗时，不含 IP、订阅或节点信息。复制后，其他应用可能读取剪贴板。" to SupplementalTranslation("Contains only test status and timing, without IP addresses, subscriptions or nodes. Other apps may read the clipboard after copying.", "僅包含檢測狀態與耗時，不含 IP、訂閱或節點資訊。複製後，其他應用可能讀取剪貼簿。", "検査の状態と時間のみを含み、IP、購読、ノード情報は含みません。コピー後は他のアプリがクリップボードを読める場合があります。", "Contient uniquement les états et durées des tests, sans IP, abonnements ni nœuds. D’autres applications peuvent lire le presse-papiers après la copie.", "Enthält nur Teststatus und Laufzeiten, keine IP-Adressen, Abonnements oder Knoten. Andere Apps können die Zwischenablage nach dem Kopieren lesen."),
    "出口已失效，请重新选择" to SupplementalTranslation("Exit unavailable; select again", "出口已失效，請重新選擇", "出口が無効です。再選択してください", "Sortie indisponible, choisissez à nouveau", "Ausgang ungültig; erneut auswählen"),
    "配置出口" to SupplementalTranslation("Configured exit", "設定出口", "設定された出口", "Sortie configurée", "Konfigurierter Ausgang"),
    "隧道已建立" to SupplementalTranslation("Tunnel established", "隧道已建立", "トンネル接続済み", "Tunnel établi", "Tunnel aufgebaut"),
    "出口已验证" to SupplementalTranslation("Exit verified", "出口已驗證", "出口を確認済み", "Sortie vérifiée", "Ausgang geprüft"),
    "等待网络" to SupplementalTranslation("Waiting for network", "等待網路", "ネットワークを待機中", "En attente du réseau", "Warten auf Netzwerk"),
    "正在恢复" to SupplementalTranslation("Recovering", "正在復原", "復旧中", "Récupération", "Wiederherstellung"),
    "隧道状态不等于出口可达；可在网络与隐私检测中验证。" to SupplementalTranslation("A tunnel does not prove reachability. Verify in Network & privacy.", "隧道狀態不等於出口可達；可在網路與隱私檢測中驗證。", "トンネル接続は到達性の証明ではありません。ネットワークとプライバシーで確認してください。", "Un tunnel ne prouve pas la connectivité. Vérifiez dans Réseau et confidentialité.", "Ein Tunnel belegt keine Erreichbarkeit. Unter Netzwerk und Datenschutz prüfen."),
    "历史结果：节点、网络或配置已变化，请重新检测。" to SupplementalTranslation("Historical results: connection context changed. Run a new check.", "歷史結果：節點、網路或設定已變更，請重新檢測。", "過去の結果：接続条件が変わりました。再測定してください。", "Anciens résultats : connexion modifiée. Relancez le test.", "Frühere Ergebnisse: Verbindung geändert. Erneut prüfen."),
    "网络尚未就绪，请连接并等待恢复后重试" to SupplementalTranslation("Network not ready. Connect and wait for recovery before retrying.", "網路尚未就緒，請連線並等待恢復後重試", "ネットワークの準備ができていません。接続と復旧を待って再試行してください。", "Réseau indisponible. Connectez-vous et attendez le rétablissement avant de réessayer.", "Netzwerk nicht bereit. Verbinden und Wiederherstellung abwarten, dann erneut versuchen."),
    "预览已失效，请重新获取" to SupplementalTranslation("Preview expired. Fetch it again.", "預覽已失效，請重新取得", "プレビューの期限が切れました。再取得してください。", "Aperçu expiré. Rechargez-le.", "Vorschau abgelaufen. Erneut laden."),
    "订阅已变化，请重新预览" to SupplementalTranslation("Subscription changed. Review it again.", "訂閱已變更，請重新預覽", "購読が変更されました。再確認してください。", "Abonnement modifié. Vérifiez à nouveau.", "Abonnement geändert. Erneut prüfen."),
    "请先核对异常节点数量变化" to SupplementalTranslation("Review the unusual node count change first.", "請先核對異常節點數量變化", "ノード数の大きな変化を確認してください。", "Vérifiez la variation inhabituelle du nombre de nœuds.", "Bitte die ungewöhnliche Änderung der Knotenanzahl prüfen."),
    "订阅节点有变化，请在详情中预览后更新" to SupplementalTranslation("Nodes changed. Review the update in subscription details.", "訂閱節點有變更，請在詳情中預覽後更新", "ノードが変わりました。購読詳細で更新を確認してください。", "Nœuds modifiés. Vérifiez la mise à jour dans les détails.", "Knoten geändert. Aktualisierung in den Abonnementdetails prüfen."),
    "内核实际命中" to SupplementalTranslation("Actual core matches", "核心實際命中", "コアの実際の一致", "Correspondances du cœur", "Tatsächliche Core-Treffer"),
    "读取实际命中" to SupplementalTranslation("Read actual matches", "讀取實際命中", "実際の一致を取得", "Lire les correspondances", "Tatsächliche Treffer lesen"),
    "当前无活跃连接证据" to SupplementalTranslation("No active connection evidence", "目前無活躍連線證據", "現在の接続証拠はありません", "Aucune preuve de connexion active", "Keine aktiven Verbindungsnachweise"),
    "仅手动读取当前活跃连接；不保存域名或地址，结果两分钟后清除。链路按内核返回顺序显示。" to SupplementalTranslation("Manual active-connection snapshot only. No domains or addresses stored. Cleared after two minutes. Chain order comes from the core.", "僅手動讀取目前活躍連線；不保存網域或位址，結果兩分鐘後清除。鏈路按核心回傳順序顯示。", "手動の接続スナップショットのみ。ドメインとアドレスは保存せず、2分後に消去します。経路順序はコアの返却順です。", "Instantané manuel uniquement. Aucun domaine ni adresse conservé. Effacé après deux minutes. Ordre fourni par le cœur.", "Nur manuelle Momentaufnahme. Keine Domains oder Adressen gespeichert. Löschung nach zwei Minuten. Kettenreihenfolge laut Core."),
    "无法读取内核连接；请连接后重试，不使用配置推测代替结果。" to SupplementalTranslation("Core connection evidence unavailable. Connect and retry; configuration guesses are not substituted.", "無法讀取核心連線；請連線後重試，不以設定推測代替結果。", "コアの接続証拠を取得できません。接続後に再試行してください。設定からの推測は表示しません。", "Preuve indisponible. Connectez-vous et réessayez ; aucune déduction de configuration ne la remplace.", "Core-Nachweis nicht verfügbar. Verbinden und erneut versuchen; keine Vermutungen als Ersatz."),
    "订阅更新预览" to SupplementalTranslation("Subscription update preview", "訂閱更新預覽", "購読更新プレビュー", "Aperçu de mise à jour", "Abonnement-Updatevorschau"),
    "新增节点" to SupplementalTranslation("Added nodes", "新增節點", "追加ノード", "Nœuds ajoutés", "Neue Knoten"),
    "移除节点" to SupplementalTranslation("Removed nodes", "移除節點", "削除ノード", "Nœuds supprimés", "Entfernte Knoten"),
    "受影响的应用规则" to SupplementalTranslation("Affected app rules", "受影響的應用程式規則", "影響を受けるアプリルール", "Règles d’applications concernées", "Betroffene App-Regeln"),
    "确认更新" to SupplementalTranslation("Confirm update", "確認更新", "更新を確認", "Confirmer la mise à jour", "Update bestätigen"),
    "订阅已安全更新" to SupplementalTranslation("Subscription updated safely", "訂閱已安全更新", "購読を安全に更新しました", "Abonnement mis à jour", "Abonnement sicher aktualisiert"),
    "默认出口将失效，需要重新选择。" to SupplementalTranslation("Default exit will become invalid. Select it again.", "預設出口將失效，需要重新選擇。", "既定の出口が無効になります。再選択してください。", "La sortie par défaut deviendra invalide. Sélectionnez-la à nouveau.", "Standardausgang wird ungültig. Erneut auswählen."),
    "我已核对异常数量变化，允许本次更新" to SupplementalTranslation("I reviewed the unusual count change and allow this update", "我已核對異常數量變更，允許本次更新", "異常な件数変更を確認し、今回の更新を許可します", "J’ai vérifié le changement inhabituel et j’autorise cette mise à jour", "Ungewöhnliche Anzahl geprüft; dieses Update zulassen"),
    "确认前不会覆盖现有订阅；改名无法可靠确认时按新增和移除展示。" to SupplementalTranslation("Nothing is overwritten before confirmation. Uncertain renames appear as additions and removals.", "確認前不會覆蓋既有訂閱；無法可靠確認改名時按新增和移除展示。", "確認前に上書きしません。確実でない名称変更は追加と削除で表示します。", "Aucun écrasement avant confirmation. Les renommages incertains apparaissent comme ajouts et suppressions.", "Kein Überschreiben vor Bestätigung. Unsichere Umbenennungen erscheinen als Hinzufügen und Entfernen."),
    "未连接：仅保留配置，尚未验证运行效果" to SupplementalTranslation("Disconnected: settings saved, runtime behavior not verified", "未連線：僅保留設定，尚未驗證執行效果", "未接続：設定のみ保存。動作は未検証です", "Déconnecté : paramètres enregistrés, fonctionnement non vérifié", "Getrennt: Einstellungen gespeichert, Laufzeitverhalten nicht geprüft"),
    "检测时间" to SupplementalTranslation("Measured at", "檢測時間", "測定日時", "Mesuré le", "Messzeitpunkt"),
    "结果仅代表检测当时的出口；切换节点或网络后请重测。刷新期间保留上次结果，不代表本次成功。" to SupplementalTranslation("Results describe the exit at measurement time. Retest after changing nodes or networks. Previous results remain during refresh and do not indicate a new success.", "結果僅代表檢測當時的出口；切換節點或網路後請重測。更新期間保留上次結果，不代表本次成功。", "結果は測定時の出口を示します。ノードやネットワーク変更後は再測定してください。更新中の前回結果は今回の成功を意味しません。", "Résultats de la sortie au moment du test. Retestez après un changement de nœud ou de réseau. Les anciens résultats restent visibles pendant l’actualisation, sans confirmer un nouveau succès.", "Ergebnisse gelten für den Ausgang zum Messzeitpunkt. Nach Knoten- oder Netzwechsel erneut testen. Während der Aktualisierung sichtbare alte Ergebnisse bestätigen keinen neuen Erfolg."),
    "仅检测应用内 WebView，不代表 Chrome 或其他应用的隐私状态。" to SupplementalTranslation("Only the in-app WebView is tested, not Chrome or other apps.", "僅檢測應用程式內 WebView，不代表 Chrome 或其他應用程式的隱私狀態。", "アプリ内 WebView のみを検査します。Chrome や他のアプリのプライバシー状態は示しません。", "Seule la WebView intégrée est testée, pas Chrome ni les autres applications.", "Getestet wird nur die integrierte WebView, nicht Chrome oder andere Apps."),
    "复制诊断摘要" to SupplementalTranslation("Copy diagnostic summary", "複製診斷摘要", "診断概要をコピー", "Copier le résumé du diagnostic", "Diagnoseübersicht kopieren"),
    "诊断摘要已复制" to SupplementalTranslation("Diagnostic summary copied", "診斷摘要已複製", "診断概要をコピーしました", "Résumé du diagnostic copié", "Diagnoseübersicht kopiert"),
    "无法访问剪贴板" to SupplementalTranslation("Clipboard unavailable", "無法存取剪貼簿", "クリップボードを利用できません", "Presse-papiers inaccessible", "Zwischenablage nicht verfügbar"),
    "仅包含版本、系统 API、恢复状态和错误代码；不包含原始日志、地址或订阅数据。" to SupplementalTranslation("Includes version, Android API, recovery status and error codes only. No raw logs, addresses or subscription data.", "僅包含版本、系統 API、復原狀態和錯誤代碼；不包含原始日誌、位址或訂閱資料。", "バージョン、Android API、復旧状態、エラーコードのみ。生ログ、アドレス、購読データは含みません。", "Version, API Android, état de récupération et codes d’erreur uniquement. Aucun journal brut, adresse ou donnée d’abonnement.", "Nur Version, Android-API, Wiederherstellungsstatus und Fehlercodes. Keine Rohprotokolle, Adressen oder Abonnementdaten."),
    ...
    "TLS 握手失败" to SupplementalTranslation("TLS handshake failed", "TLS 交握失敗", "TLS ハンドシェイク失敗", "Échec de la négociation TLS", "TLS-Handshake fehlgeschlagen"),
    "主机解析失败" to SupplementalTranslation("Host resolution failed"),
    "连接被拒绝" to SupplementalTranslation("Connection refused", "連線遭拒", "接続が拒否されました", "Connexion refusée", "Verbindung abgelehnt"),

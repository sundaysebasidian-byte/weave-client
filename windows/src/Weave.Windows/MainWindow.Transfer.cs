...
public sealed partial class MainWindow
{
    ...
    private async void ShareLan_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (selected.Length == 0) { MessageText.Text = "先在订阅列表勾选要同步的订阅"; return; }
    ...
    {
        try { await StopShareAsync(); } catch { MessageText.Text = "分享已关闭"; }
    }
    private void RefreshAddresses_Click(object sender, RoutedEventArgs e)
    {
        ...
        catch (System.Net.NetworkInformation.NetworkInformationException) { MessageText.Text = "无法读取网络接口"; }
    }
    private void CopyShareLink_Click(object sender, RoutedEventArgs e)
    {
        ...
        MessageText.Text = "已复制一次性链接，已请求禁用剪贴板历史和跨设备漫游；不要发到公开群聊";
    }
    private async void ReceiveLan_Click(object sender, RoutedEventArgs e)
    {
        await RunActionAsync(async () =>
        {
            ...
            try
            {
                if (ReceiveCode.Text.Trim() != link.ConfirmationCode()) throw new InvalidDataException("请填写发送设备显示的六位核对码");
    ...
    private async void ConfirmTransfer_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (selected.Length == 0) { MessageText.Text = "请先接收并选择订阅"; return; }
        await RunActionAsync(async () =>
        {
    ...
    private async void QrFile_Click(object sender, RoutedEventArgs e)
    {
        await RunActionAsync(async () =>
        {
            var path = DesktopFilePicker.Open(WindowNative.GetWindowHandle(this), "二维码图片\0*.png;*.jpg;*.jpeg;*.bmp;*.webp\0\0");
    ...
    private void AcceptQr(string value)
    {
        ...
        else if (Uri.TryCreate(value, UriKind.Absolute, out var uri) && uri.Scheme == "https")
        {
            ...
            MessageText.Text = "已识别订阅链接，请确认后点击导入";
        }
        else throw new InvalidDataException("二维码不是 HTTPS 订阅或 Weave 加密互传链接");
    }
    ...
    private async void CameraQr_Click(object sender, RoutedEventArgs e)
    {
        ...
        panel.Children.Add(new TextBlock { Text = "对准订阅或 Weave 互传二维码。图像只在本机识别，不保存、不上传。", TextWrapping = TextWrapping.Wrap });
        var dialog = new ContentDialog { Title = "扫描二维码", Content = panel, CloseButtonText = "关闭", XamlRoot = RootGrid.XamlRoot };
        string? scanned = null;
        var renderPending = 0;
        var active = true;
        ...
        { MessageText.Text = "无法扫描：请检查 Windows 摄像头权限，或改用二维码图片 / 链接。"; }

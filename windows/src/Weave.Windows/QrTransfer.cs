...
internal static class QrTransfer
{
    ...
    {
        if (new FileInfo(path).Length > 20 * 1024 * 1024) throw new InvalidDataException("二维码图片超过 20 MiB");
        var file = await StorageFile.GetFileFromPathAsync(path);
        using var stream = await file.OpenReadAsync();
        var decoder = await BitmapDecoder.CreateAsync(stream);
        if ((long)decoder.PixelWidth * decoder.PixelHeight > 24_000_000) throw new InvalidDataException("二维码图片像素过大");
        ...
        return await Task.Run(() => Decode(data, (int)width, (int)height)) ?? throw new InvalidDataException("图片中未识别到二维码");
    }
